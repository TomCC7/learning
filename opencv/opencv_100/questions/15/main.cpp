...
/**
 * @brief sobel filter based on standard 3*3 grid
 *
 * @param src
 * @param dst
 * @param vertical bool
 */
void SobelFilter(const Mat &src, Mat &dst, bool vertical = false)
{
...
int main()
{
    ...
    Mat out;
    SobelFilter(in, out, true);
    imshow("sobel filter", out);
    imwrite("sobel_filter.jpg", out);
    while (waitKey(0) != 27) {}
    destroyAllWindows();
}