...
/**
 * @brief laplacian filter based on standard 3*3 grid
 *
 * @param src
 * @param dst
 * @param vertical bool
 */
void LaplacianFilter(const Mat &src, Mat &dst)
{
    CV_Assert(src.channels() == 1);
    Vec<Vec3i, 3> filter = {{0, 1, 0}, {1, -4, 1}, {0, 1, 0}}; // filter matrix
    dst = src.clone();
...
int main()
{
    ...
    Mat out;
    LaplacianFilter(in, out);
    imshow("Laplacian filter", out);
    imshow("gray", in);
    imwrite("laplacian.jpg", out);
    while (waitKey(0) != 27) {}
    destroyAllWindows();
}