 ...
 * @file main.cpp
 * @author cc
 * @brief OpenCV 100Wen: 31 Afine Transformations-rotation
 * @date 2021-01-18
 */
#include <cmath>
#include <iostream>
...
void af_lean(const cv::Mat &src, cv::Mat &dst, double x = 0, double y = 0)
{
    affine(src, dst, 1, x / src.cols, y / src.rows, 1, 0, 0);
}
...
int main()
{
    ...
    Mat out;
    af_lean(in, out, 30, 30);
    imshow("affine-lean", out);
    imshow("origin", in);
    imwrite("answers/affine_lean.jpg", out);
    while (waitKey(0) != 27) {}
    destroyAllWindows();
}
