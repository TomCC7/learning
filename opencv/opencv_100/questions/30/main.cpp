 ...
 * @file main.cpp
 * @author cc
 * @brief OpenCV 100Wen: 30 Afine Transformations-rotation
 * @date 2021-01-18
 */
#include <cmath>
#include <iostream>
 ...
 */
void af_rotate(const cv::Mat &src, cv::Mat &dst, double a)
{
    a = a / 180. * M_PI;
    cv::Point2d mid((double)src.rows / 2, (double)src.cols / 2);
    cv::Point2d mid_new(mid.x * cos(a) - mid.y * sin(a), mid.x * sin(a) + mid.y * cos(a));
    cv::Point2d offset = mid - mid_new;
    cout << offset.x << ',' << offset.y << endl;
    affine(src, dst, cos(a), -sin(a), sin(a), cos(a), 0, 0);
}
